    ...
    for data in freights_data:
        try:
            ...
            freight = models.Freight(
                ...
                weight_kg=data.get("weight_kg"),  # Save parsed weight
                ldm=data.get("ldm"),
...
async def run_scraper_task(
    ...
    try:
        ...
        if truck_id:
             ...
             if truck:
                 if truck.max_payload:
                     filters["max_weight"] = truck.max_payload
                 # If origin missing but truck has location (and GPS failed/not used above), use last known
                 if "origin" not in filters and truck.last_known_location:
                      filters["origin"] = truck.last_known_location
