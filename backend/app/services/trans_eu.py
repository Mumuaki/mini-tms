...
class TransEuScraper:
    ...
    async def _apply_filters(self, filters: Dict):
        ...
        if "origin" in filters:
            await self._retry(lambda: self._set_location("Загрузка", TransEuSelectors.LOADING_CTX, filters["origin"]))
        if "destination" in filters:
            await self._retry(lambda: self._set_location("Разгрузка", TransEuSelectors.UNLOADING_CTX, filters["destination"]))
        ...
        # 4. Weight (Step 16)
        # Default max weight to 0.9t if not specified (User Requirement)
        # Assuming we want to set "To" 0.9
        await self._set_weight(0.9)
    ...
    async def _set_location(self, label: str, ctx_selector: str, value: str):
        ...
        try:
           ...
           if await clear_btn.is_visible(): await clear_btn.click()
        except: pass
