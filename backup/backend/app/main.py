...
def get_truck_location(truck_id: int, db: Session = Depends(database.get_db)):
    """
    Get current real-time location of a truck from GPS Dozor.
    """
    from .services.gps_service import GpsDozorService
    from .services.google_maps import GoogleMapsService
    ...
    if not truck.gps_vehicle_code:
        raise HTTPException(status_code=400, detail="GPS vehicle code not set")
    ...
    try:
        ...
        coords = gps_service.get_vehicle_location(truck.gps_vehicle_code)
        if not coords:
             # Return last known if live fetch fails, or error?
